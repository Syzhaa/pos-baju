...
import { Plus, Trash2, DollarSign, Printer, Search } from "lucide-react";
...
const ReceiptToPrint = forwardRef<HTMLDivElement, { transaction: Transaction | null }>(
  ({ transaction }, ref) => {
...
export default function KasirPage() {
  ...
  const handlePrint = useReactToPrint({
    // @ts-expect-error - TypeScript doesn't recognize "content" correctly here
    content: () => receiptRef.current,
  });

  const availableSizes = selectedProduct
    ? Object.keys(selectedProduct.stock).filter(
  ...
  return (
      ...
      {/* Komponen lainnya tetap sama seperti sebelumnya */}
      {/* ... */}
      <div className="mt-8">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 p-4">
          <div className="bg-slate-100 p-6 rounded-xl shadow-2xl w-full max-w-sm">
            <div className="bg-white max-h-96 overflow-y-auto">
              <h3 className="text-lg font-bold text-center mb-2 p-4 text-slate-900">
                Transaksi Berhasil!
              </h3>
            ...
            <div className="mt-4 text-center text-xs text-slate-600">
              <p>
                Pilih &quot;Save as PDF&quot; atau printer Anda (Bluetooth/Kabel) pada dialog
                cetak.
              </p>
            </div>
            <div className="flex gap-4 mt-2">
              <button
                ...
                className="w-full bg-slate-300 text-slate-800 p-2 rounded-lg hover:bg-slate-400 transition-colors"
              >
                Tutup
              </button>
              <button
